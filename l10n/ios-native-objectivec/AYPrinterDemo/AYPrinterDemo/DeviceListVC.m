...
- (void)bleHelperDidUpdateState:(CBManagerState)state {
    ...
    NSLog(@"%@", AYLocalizedString(@"蓝牙异常"));
}

- (void)bleHelperDiscoverPeripheral:(AYPrinter *)printer {
    //    float dist = [self calcDistByRSSI:RSSI.intValue]; // 根据 RSSI 估算距离 / Estimate distance from RSSI.
    //    NSLog(@"%@ 的距离是： %f", peripheral.name, dist); // 调试距离输出 / Debug distance output.
    // NSLog(@"self.searchBar.text： %@", self.searchBar.text); // 调试搜索关键词 / Debug current search keyword.
    ...
    if (!isExist) {
        NSLog(AYLocalizedString(@"发现设备 name：%@, uuid: %@, mac: %@"), printer.name, printer.uuid, printer.mac);
...
- (void)bleHelperDidChangeConnectState:(BleState)state peripheral:(CBPeripheral *)peripheral {
    ...
    if (state == BleStateConnected) {
        UIAlertController *alertController = [UIAlertController alertControllerWithTitle:AYLocalizedString(@"指令类型") message:nil preferredStyle:UIAlertControllerStyleAlert];
      ...
      UIAlertAction *rgbAction = [UIAlertAction actionWithTitle:AYLocalizedString(@"彩喷") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {

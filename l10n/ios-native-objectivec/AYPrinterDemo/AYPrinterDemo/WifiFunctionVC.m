...
- (IBAction)setWifi:(id)sender {
  if (_nameTextF.text.length < 1) {
    NSLog(@"%@", AYLocalizedString(@"请输入wifi名称"));
    return;
  }
  if (_pwdTextF.text.length < 1) {
    NSLog(@"%@", AYLocalizedString(@"请输入wifi密码"));
    return;
  }
...
//  [self readTimeOut:1]; // 备用超时读取逻辑 / Fallback timeout-based read logic.
}
...
- (void)readafterDelay {
  ...
  switch (self.mark) {
    case WifiName:
      self.displayLabel.text = [NSString stringWithFormat:AYLocalizedString(@"wifi 名称: %@"), string];
      break;
    case WifiPassword:
      self.displayLabel.text = [NSString stringWithFormat:AYLocalizedString(@"wifi 密码: %@"), string];
      break;
    case WifiState: {
        BOOL isConnected = [string containsString:@"Connected"];
        self.displayLabel.text = [NSString stringWithFormat:AYLocalizedString(@"wifi 连接状态: %@"), isConnected ? AYLocalizedString(@"已连接") : AYLocalizedString(@"未连接")];
    }
      break;
    case WifiDHCP: {
        BOOL isDynamics = [string hasPrefix:@"1"];
        self.displayLabel.text = [NSString stringWithFormat:AYLocalizedString(@"DHCP: %@"), isDynamics ? AYLocalizedString(@"动态") : AYLocalizedString(@"静态")];
    }
      break;
    case IpGet: {
      NSLog(AYLocalizedString(@"获取ip: %@"), self.revData);
      NSArray *infos = [string componentsSeparatedByString:@"\r\n"];
      if (infos.count < 2) {
        return;
      }
      self.displayLabel.text = [NSString stringWithFormat:AYLocalizedString(@"ip: %@\n子网掩码: %@\n网关: %@"), infos[0], infos[1], infos[2]];
    }
      break;
    default:
      break;

...
public class PDFActivity extends Activity {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    bt_print_pdf.setOnClickListener(new View.OnClickListener() {
      ...
      public void onClick(View v) {
        if (bitmaps == null) {
          showMessage(getString(R.string.pdf_not_selected));
          return;
        }
      ...
      switch (requestCode) {
        case PICK_PDF:
          if (!file_path.toLowerCase().contains("pdf")) {
            showMessage(getString(R.string.select_pdf_file));
            return;
          }
          PdfUtil.showLoading(this, getString(R.string.loading));
          new Thread(() -> {
  ...
  public void imageTest(Bitmap rawBitmap) {
    if (et_width.getText().toString().isEmpty() || et_height.getText().toString().isEmpty()) {
      showMessage(getString(R.string.paper_empty_error));
      return;
    }
    ...
    if (curCmd.equals("tspl")) {
      GenericTSPL _gtspl = PrintUtil.getInstance().tspl().clear()
          .page(TPage.builder().width(pageWidth).height(pageHeight).build());
      ...
      GenericCPCL _gcpcl = PrintUtil.getInstance().cpcl()
          .page(CPage.builder().width(pageWidth * 8).height(pageHeight * 8).build())
  ...
  private void safeWrite(PSDK psdk) {
    try {
      ...
      e.printStackTrace();
      showMessage(getString(R.string.send_failed_reconnect));
    }
  }

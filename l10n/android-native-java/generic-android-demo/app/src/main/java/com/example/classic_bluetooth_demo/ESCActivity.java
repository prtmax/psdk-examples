...
public class ESCActivity extends Activity {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    setThickness.setOnClickListener(new View.OnClickListener() {
      ...
      public void onClick(View v) {
        if (thickness.getText() != null && thickness.length() != 0) {
          ...
          int thicknessValue = Integer.valueOf(nongduzhi);
          GenericESC _gesc = PrintUtil.getInstance().esc().thickness(thicknessValue);
          safeWrite(_gesc);
        } else {
          Util.show(ESCActivity.this, getString(R.string.input_density));
        }
      }
    });
    ...
    bottom_stock.setOnClickListener(new View.OnClickListener() {
      ...
      public void onClick(View v) {
        ...
        GenericESC _gesc = PrintUtil.getInstance().esc()
            .paperType(EPaperType.builder().type(EPaperType.Type.CONTINUOUS_REEL_PAPER).build());
        safeWrite(_gesc);
      }
    });
    bottom_label.setOnClickListener(new View.OnClickListener() {
      ...
      public void onClick(View v) {
        readMark = ReadMark.OPERATE_PAPERTYPE;
        GenericESC _gesc = PrintUtil.getInstance().esc()
            .paperType(EPaperType.builder().type(EPaperType.Type.FOLDED_BLACK_LABEL_PAPER).build());
        safeWrite(_gesc);
      }
    });
    ...
    updatePrinterButton.setOnClickListener(new View.OnClickListener() {
      ...
      public void onClick(View v) {
        ...
        UpdatePrinterESC updatePrinter = new UpdatePrinterESC(connection,
            Util.readResources(ESCActivity.this, R.raw.x8prov1099hd2024423), otaHandler);
    ...
    public void handleMessage(Message msg) {
      ...
      switch (transactionTypeEnum) {
        ...
        case MSG_OTA_DATA_COMMAND_SEND_FAILED_PRINTER: {
          ...
          Toast.makeText(ESCActivity.this, getString(R.string.upgrade_failed), Toast.LENGTH_SHORT).show();
          break;
        }
        ...
        case MSG_OTA_FINISHED_PRINTER: {
          ...
          Toast.makeText(ESCActivity.this, getString(R.string.upgrade_finished), Toast.LENGTH_SHORT).show();
          break;
        }
      }
    }
  ...
  public void onPrintStatus(byte[] bytes) {
    ...
    if (bytes[1] == 0x00) {
      Log.e(TAG, "正常");
      Util.show(ESCActivity.this, getString(R.string.printer_normal));
    }
    if ((bytes[1] & 0x01) == 0x01) {
      Log.e(TAG, "过热");
      Util.show(ESCActivity.this, getString(R.string.printer_overheat));
    }
    if ((bytes[1] & 0x02) == 0x02) {
      Log.e(TAG, "开盖");
      Util.show(ESCActivity.this, getString(R.string.printer_open));
      // 开盖 缺纸 固件会取消打印
      onCancelPrint();
    }
    if ((bytes[1] & 0x04) == 0x04) {
      Log.e(TAG, "缺纸");
      Util.show(ESCActivity.this, getString(R.string.paper_empty));
      // 开盖 缺纸 固件会取消打印
      onCancelPrint();
    }
    if ((bytes[1] & 0x08) == 0x08) {
      Log.e(TAG, "低压");
      Util.show(ESCActivity.this, getString(R.string.printer_low_battery));
    }
  }
  ...
  public void onPaperError(byte[] bytes) {
    switch (bytes[1]) {
      case 0x01:
        ...
        Util.show(ESCActivity.this, getString(R.string.black_mark_paper));
        break;
      case 0x02:
        Log.e(TAG, "onPaperError: 连续卷筒纸");
        Util.show(ESCActivity.this, getString(R.string.continuous_paper));
        break;
      case 0x03:
        Log.e(TAG, "onPaperError: 不干胶缝隙纸");
        Util.show(ESCActivity.this, getString(R.string.label_paper));
        break;
    }
    onCancelPrint();
  }
  ...
  public void onStartOrStopSend(byte[] bytes) {
    switch (bytes[1]) {
      case 0x01:
        ...
        Util.show(ESCActivity.this, getString(R.string.printer_paused) + ": FD 01");
        onCancelPrint();
        break;
      case 0x02:
        Log.e(TAG, "onStartOrStopSend: 继续开始命令");
        Util.show(ESCActivity.this, getString(R.string.printer_printing) + ": FD 02");
        break;
    }
  }
  ...
  public void onBatteryStatus(byte[] bytes) {
    switch (bytes[1]) {
      case 0x00:
        Util.show(ESCActivity.this, getString(R.string.battery_status_normal));
        break;
      case 0x01:
        Util.show(ESCActivity.this, getString(R.string.battery_status_low));
        break;
      case 0x02:
        Util.show(ESCActivity.this, getString(R.string.battery_status_charging));
        break;
      case 0x03:
        Util.show(ESCActivity.this, getString(R.string.battery_status_complete));
        break;
    }
  }
  ...
  public void onReceive(byte[] bytes) {
    ...
    switch (readMark) {
      case OPERATE_STATUS:// 主动查询状态的时候走这里
        readMark = ReadMark.NONE;
        if (bytes.length == 1) {
          StringBuilder s = new StringBuilder(getString(R.string.printer_status) + ": ");
          boolean isok = true;
          if ((bytes[0] & 0x01) == 0x01) {
            s.append(getString(R.string.printer_printing)).append(" ");
            isok = false;
          }
          if ((bytes[0] & 0x02) == 0x02) {
            s.append(getString(R.string.printer_open)).append(" ");
            isok = false;
          }
          if ((bytes[0] & 0x04) == 0x04) {
            s.append(getString(R.string.paper_empty)).append(" ");
            isok = false;
          }
          if ((bytes[0] & 0x08) == 0x08) {
            s.append(getString(R.string.printer_low_battery)).append(" ");
            isok = false;
          }
          if ((bytes[0] & 0x10) == 0x10) {
            s.append(getString(R.string.printer_overheat)).append(" ");
            isok = false;
          }
          if (isok) {
            s.append(getString(R.string.printer_normal));
          }
          Util.show(ESCActivity.this, s.toString());
        }
      ...
      case OPERATE_BATVOL:
        readMark = ReadMark.NONE;
        if (bytes.length == 2) {
          String s = getString(R.string.battery_level_prefix) + (int) bytes[1];
      ...
      case OPERATE_TIME:
        readMark = ReadMark.NONE;
        String time = getString(R.string.shutdown_time, Long.parseLong(Util.ByteArrToHex(bytes), 16));
      ...
      case OPERATE_NFC_PAPER:
        readMark = ReadMark.NONE;
        String paper_info_str;
        String[] infos;
        try {
          paper_info_str = new String(bytes, "GB2312");
          infos = paper_info_str.split("\\|");
          if (infos.length >= 4) {
            // 颜色:1 代表白色
      ...
      case OPERATE_NFC_UID:
        readMark = ReadMark.NONE;
        String paper_uid_str = Util.ByteArrToHex(bytes);
        Util.show(ESCActivity.this, paper_uid_str);
        Log.e(TAG, "纸张uid: " + paper_uid_str);
        break;
      ...
      case OPERATE_PRINT:
        readMark = ReadMark.NONE;
        if (Util.ByteArrToHex(bytes).contains("4F4B") || Util.ByteArrToHex(bytes).contains("AA")) {// 打印成功
                                                                                                   // 比如标签纸打印完成会返回4F4B
                                                                                                   // 连续纸打印完成会返回AA
      ...
      default:
        ...
        if (Util.ByteArrToHex(bytes).equals("4F4B")) {// 固件一些其他的返回在这里处理
          Util.show(ESCActivity.this, getString(R.string.success));
          Log.e(TAG, "成功");
        }
        if (new String(bytes).equals("ER")) {
          Util.show(ESCActivity.this, getString(R.string.failed));
          Log.e(TAG, "失败");
        }
        break;
    }

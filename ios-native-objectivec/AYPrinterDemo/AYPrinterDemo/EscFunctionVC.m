...
@interface EscFunctionVC ()<UITextFieldDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate>
...
#pragma mark - AI打印机专属功能
/**
 * 设置待机样式
 *
 * @param model 待机样式  1 图片  2 日历
 */
- (IBAction)setStandbyMode {
  if (self.isPrinting) return;
  
  UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"设置待机样式" message:nil preferredStyle:UIAlertControllerStyleAlert];
  UIAlertAction *imageAction = [UIAlertAction actionWithTitle:@"图片" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
    [self.esc clean];
    [self.esc setStandbyMode:1];
    [self.bleHelper writeCommands:self.esc.commands];
    self.displayLabel.text = @"设置待机样式：图片";
  }];
  UIAlertAction *calendarAction = [UIAlertAction actionWithTitle:@"日历" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
    [self.esc clean];
    [self.esc setStandbyMode:2];
    [self.bleHelper writeCommands:self.esc.commands];
    self.displayLabel.text = @"设置待机样式：日历";
  }];
 ...
 */
- (IBAction)setSystemLanguage {
  if (self.isPrinting) return;
  
  UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"设置系统语言" message:nil preferredStyle:UIAlertControllerStyleAlert];
  UIAlertAction *englishAction = [UIAlertAction actionWithTitle:@"英文" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
    [self.esc clean];
    [self.esc setSystemLanguage:1];
    [self.bleHelper writeCommands:self.esc.commands];
    self.displayLabel.text = @"设置系统语言：英文";
  }];
  UIAlertAction *chineseAction = [UIAlertAction actionWithTitle:@"中文" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
    [self.esc clean];
    [self.esc setSystemLanguage:2];
    [self.bleHelper writeCommands:self.esc.commands];
    self.displayLabel.text = @"设置系统语言：中文";
  }];
  UIAlertAction *cancelAction = [UIAlertAction actionWithTitle:@"取消" style:UIAlertActionStyleCancel handler:nil];
...
- (IBAction)getSystemLanguage {
  ...
  self.bleHelper.onDataReceived = ^(NSData *data) {
    ...
    if (data.length >= 2) {
      ...
      NSString *langStr = lang == 0 ? @"中文" : [NSString stringWithFormat:@"英文(%d)", lang];
      weakSelf.displayLabel.text = [NSString stringWithFormat:@"当前语言: %@", langStr];
      NSLog(@"当前语言: %@", langStr);
    } else {
      weakSelf.displayLabel.text = [NSString stringWithFormat:@"当前语言: %@", [data toHexString]];
    }
  };
  
  [self.esc clean];
  [self.esc getSystemLanguage];
  [self.bleHelper writeCommands:self.esc.commands];
}
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary<UIImagePickerControllerInfoKey, id> *)info {
  ...
  if (!originalImage) {
    self.displayLabel.text = @"无法解码图片";
    return;
  }
  
  // 缩放到目标分辨率 792x528
  CGSize targetSize = CGSizeMake(792, 528);
  ...
  if (!scaledImage) {
    self.displayLabel.text = @"图片缩放失败";
    return;
  }
  
  [self.esc clean];
  [self.esc setStandbyImage:scaledImage];
  [self.bleHelper writeCommands:self.esc.commands];
  self.displayLabel.text = @"待机图片发送完成 (792x528)";
}
 ...
 * 设置日历图案
 */
- (IBAction)setCalendarMode {
  if (self.isPrinting) return;
  
  UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"选择日历图案类型" message:nil preferredStyle:UIAlertControllerStyleAlert];
  UIAlertAction *pattern1Action = [UIAlertAction actionWithTitle:@"图案1" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
    [self.esc clean];
    [self.esc setCalendarMode:1];
    [self.bleHelper writeCommands:self.esc.commands];
    self.displayLabel.text = @"已发送设置日历样式(图案1)";
  }];
  UIAlertAction *pattern2Action = [UIAlertAction actionWithTitle:@"图案2" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
    [self.esc clean];
    [self.esc setCalendarMode:2];
    [self.bleHelper writeCommands:self.esc.commands];
    self.displayLabel.text = @"已发送设置日历样式(图案2)";
  }];
  UIAlertAction *pattern3Action = [UIAlertAction actionWithTitle:@"图案3" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
    [self.esc clean];
    [self.esc setCalendarMode:3];
    [self.bleHelper writeCommands:self.esc.commands];
    self.displayLabel.text = @"已发送设置日历样式(图案3)";
  }];
  UIAlertAction *cancelAction = [UIAlertAction actionWithTitle:@"取消" style:UIAlertActionStyleCancel handler:nil];
  
  [alertController addAction:pattern1Action];
  [alertController addAction:pattern2Action];
  [alertController addAction:pattern3Action];
  [alertController addAction:cancelAction];
  [self presentViewController:alertController animated:YES completion:nil];
}
 ...
 * 查询设备信息
 */
- (IBAction)getDeviceInfo {
  if (self.isPrinting) return;
  __weak typeof(self) weakSelf = self;
  self.bleHelper.onDataReceived = ^(NSData *data) {
    NSString *deviceInfoRaw = [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
    if (!deviceInfoRaw) {
      deviceInfoRaw = [data toHexString];
    }
    weakSelf.displayLabel.text = [NSString stringWithFormat:@"设备信息:\n%@", deviceInfoRaw];
    NSLog(@"设备信息: %@", deviceInfoRaw);
  };
  
  [self.esc clean];
  [self.esc getDeviceInfo];
  [self.bleHelper writeCommands:self.esc.commands];
}
...
- (IBAction)bindDevice {
  ...
  __weak typeof(self) weakSelf = self;
  self.bleHelper.onDataReceived = ^(NSData *data) {
    const Byte *revByte = data.bytes;
    if ((revByte[0] == 0x4F && revByte[1] == 0x4B)) {
      weakSelf.displayLabel.text = @"已发送绑定设备";
    }
  };
  
  [self.esc clean];
  [self.esc bindDevice:@"123456" server:@"https://api.example.com" expire:@"1719980000000"];
  [self.bleHelper writeCommands:self.esc.commands];
}
